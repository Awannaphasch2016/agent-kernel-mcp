---
name: invariant
description: Identify behavioral invariants that must remain true for a given goal, generating verification checklists before claiming "done"
accepts_args: true
composition:
  - primitive
...
tuple_binding:
  slot: Check
  effect: scan
local_check: "Invariants loaded and delta calculated for all levels (4→0)"
entities:
  - invariant
  - violation
  - level
...
### Step 4.5: Behavioral Verification Protocol (Layer 0 ONLY)

**CRITICAL**: Level 0 (User/Behavior) verification MUST include runtime testing, not just code inspection.

**Principle #20 Reminder**: "Reading code ≠ Verifying code works"

**Principle #25 Reminder**: Avoid "Structural Proxy Bias" - checking if code exists ≠ checking if code works

---

#### For SAMENESS Claims (Replications)

When verifying "X replicates Y behavior":

**1. Identify Reference Behavior**
- Load reference site/app
- Document expected behavior (scroll animations, hover effects, click responses)
- Take screenshots/videos as reference

**2. Execute Runtime Test**
```bash
# Use Playwright to interact with implementation
# Example: Testing scroll-triggered animations
playwright_test:
  - Navigate to page (localhost or deployed URL)
  - Scroll to section
  - Verify element state changes (opacity, transform, classes)
  - Capture screenshots of animated state
```

**3. Compare Behavior**
- Visual comparison (do animations look the same?)
- Timing comparison (do interactions respond at same speed?)
- State comparison (does element state change correctly?)

**4. Check Implementation** (Secondary)
- Verify behavior-triggering code exists (IntersectionObserver, event handlers)
- Verify CSS animations/transitions defined
- Verify classes applied conditionally based on state

**Anti-Pattern to Avoid**: Checking only step 4 (implementation) without steps 1-3 (runtime)

**Example from Session**:
- ❌ WRONG: "animate-marquee CSS exists in tailwind.config.js" → Concluded "animations work"
- ✅ RIGHT: "Scrolled page, observed timeline steps animate with opacity 0→1 and translateY 30px→0" → Verified actual behavior

---

#### For FUNCTIONALITY Claims (New Features)

When verifying "X functionality works":

**1. Define Expected Outcome**
- What should happen when user performs action?
- What edge cases exist?

**2. Execute User Workflow**
```bash
# Manual or automated test
- Perform user action (click button, submit form, etc.)
...
- Boundary conditions (empty state, max capacity)
...
| Domain | Runtime Verification Tool | What to Verify |
|--------|--------------------------|----------------|
| **Frontend** | Playwright | Scroll animations, hover effects, click handlers, form submissions |
| **API** | curl/Postman | HTTP requests, response codes, payload structure |
| **Lambda** | AWS CLI test invoke | Function execution, return values, CloudWatch logs |
| **Database** | SQL client | Query results, data integrity, constraints |
| **Infrastructure** | AWS CLI describe | Resource existence, configuration, connectivity |

---

#### Output Format for Level 0 Verification

For each Level 0 invariant, include:

```markdown
### Level 0: User - {Specific Behavior}

**Expected Behavior**: {What should happen - from reference or requirements}

**Runtime Test Executed**:
```bash
{Playwright script, curl command, or manual test steps}
```

**Result**: ✅ Pass | ❌ Fail

**Evidence**:
- Screenshot: {path} (before/after states)
- Log output: {snippet or file path}
- Video: {path} (if complex interaction)

**Code Verification** (secondary):
- [ ] Behavior-triggering code present ({file}:{line} - e.g., IntersectionObserver setup)
- [ ] CSS/styling defined ({file}:{line} - e.g., animation keyframes)
- [ ] Conditional logic correct ({file}:{line} - e.g., state management)

**Delta**: 0 (matches expected) | N (N violations found)
```
...
Before claiming Level 0 "Delta = 0":

- [ ] Runtime test executed (not skipped)
- [ ] Actual behavior observed and documented
- [ ] Behavior matches expected (for sameness) OR works correctly (for functionality)
- [ ] Evidence collected (screenshots, logs, videos)
- [ ] Code implementation verified as secondary confirmation
- [ ] No anti-patterns committed (structural proxy bias avoided)

**If ANY checkbox unchecked**: Delta > 0 (verification incomplete)
...
- **Principle #25**: [Universal Property Verification](../../CLAUDE.md)
...
*Command: /invariant*
*Related: Principle #25 (Universal Property Verification)*

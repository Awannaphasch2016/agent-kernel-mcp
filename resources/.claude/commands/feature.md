...
**When NOT to use**:
- Exploring ideas -> Use `/specify` (exploratory, no files created)
- Quick one-off changes -> No spec needed
- Spike/proof-of-concept -> Use `/specify spike`

---

## Two-Layer Model (v2.0)

Feature specs use the **Two-Layer Model** for PRD and Invariant composition:

```
Layer 1: Feature Specs (.claude/specs/{feature}/)
├── spec.yaml         # Feature-specific metadata + refs
├── invariants.md     # Feature-specific invariants
├── constraints.md    # Feature-specific constraints
└── acceptance.md     # Feature-specific acceptance criteria

Layer 2: Shared Invariants (.claude/invariants/)
├── deployment-invariants.md   # Cross-feature deployment concerns
├── langfuse-invariants.md     # Cross-feature observability
└── {domain}-invariants.md     # Other shared concerns
```

**The `refs` Field**:

Feature specs can reference shared invariants and other specs:

```yaml
refs:
  invariants:
    - deployment      # → .claude/invariants/deployment-invariants.md
    - langfuse        # → .claude/invariants/langfuse-invariants.md
  specs:
    - meta-ads        # → .claude/specs/meta-ads/
```

**Benefits**:
- **DRY**: Define deployment invariants once, reference from all features
- **Composition**: Features can depend on other features
- **Discoverability**: `/invariant` automatically loads referenced invariants

**When to use refs vs inline invariants**:
- **refs.invariants**: For cross-feature concerns (deployment, security, observability)
- **Inline invariants.md**: For feature-specific invariants unique to this feature
...
# NEW in v2.0: References to shared invariants and other specs
# This enables the Two-Layer Model for PRD/Invariant composition
refs:
  invariants:
    - deployment      # → .claude/invariants/deployment-invariants.md
    - langfuse        # → .claude/invariants/langfuse-invariants.md
  specs: []           # → Other feature specs this depends on

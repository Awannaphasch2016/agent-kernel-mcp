...
Claude operates in one of two execution modes:

| Mode | Description | When to Use |
|------|-------------|-------------|
| **Chat** (default) | Normal conversational Claude | Quick questions, simple edits, exploration |
| **Agent Kernel** | Full Thinking Tuple protocol | Complex implementations, spec-driven work |
...
/kernel                    # Just enter mode
/kernel "implement X"      # Enter and start task
/kernel SS-19              # Enter and seed from Linear
...
- Best for quick tasks and exploration

### Agent Kernel Mode

- ALL prompts route through `/run` → `/step` → primitives
- Gradient-based termination (continues until learning stops)
- Linear integration for team visibility
- RuntimeState persistence for recovery
- Best for complex, multi-step work

**Benchmark value**: Chat mode serves as baseline to measure Agent Kernel's improvement:
```
Δ = Performance(Kernel) - Performance(Chat) = Agent Kernel value add
```

See [/kernel](kernel.md), [/chat](chat.md), [/mode](mode.md), and [CLAUDE.md - Principle #31](../CLAUDE.md).
...
### `/task-manager` - Persistent Task Management
**Purpose**: Cross-session task tracking with priority framework, rich states, and Agent Kernel integration

**Design Philosophy**: **Low cognitive energy** - just say what you want. Natural language is the primary interface.
...
# Update status
/task-manager "finished the API implementation"
/task-manager "blocked on schema, waiting for review"
/task-manager "park optimization until after launch"

# Session management
/task-manager "what should I work on next?"
/task-manager "archive completed tasks"
```

**Explicit Subcommands** (Power User):
```bash
/task-manager add "title" --priority=P1 --project=backtester
/task-manager start "title"
/task-manager done "title" --notes="completed"
/task-manager list --status=blocked
/task-manager load
```

**States**: proposed → ready → in_progress → blocked/deferred → done

**Priority Framework** (Eisenhower-inspired):
- **P0** (Crisis): Urgent + Important → Do immediately
- **P1** (Strategic): Important, not urgent → Most valuable work
- **P2** (Tactical): Urgent, not important → Batch or automate
- **P3** (Backlog): Neither → Defer or drop

**Agent Kernel Integration**:
- Tasks become **Constraints** in Thinking Tuple
- Links to `/spec` for acceptance criteria
- Syncs with `TodoWrite` for visibility
- Prompts `/journal` on completion

**Core Principle**: "Tasks bridge goals and execution" - persist context across sessions
...
### Task Management
- [/task-manager](task-manager.md) - Persistent task management with priority framework and rich states

### Execution Modes
- [/kernel](kernel.md) - Enter Agent Kernel mode (full Thinking Tuple protocol)
- [/chat](chat.md) - Return to chat mode (normal Claude)
- [/mode](mode.md) - Check or switch execution mode

---
name: reconcile
description: Converge invariant violations back to compliance by generating specific fix actions
accepts_args: true
composition:
  - primitive
...
tuple_binding:
  slot: Check
  effect: fix
local_check: "Delta converged to 0 with fixes applied and verified"
entities:
  - violation
  - fix
  - member
...
doppler secrets set SECRET_NAME "value" --project ss-automation --config dev
```
...
**Verification**:
```bash
doppler secrets get [SECRET_NAME] --project ss-automation --config dev
```

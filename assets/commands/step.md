---
name: step
description: Instantiate a Thinking Tuple - the atomic unit of disciplined reasoning that forces composition of constraints, invariants, principles, process, actions, and verification
accepts_args: true
composition:
  - primitive
...
tuple_binding:
  slot: All
  effect: orchestrate
local_check: "Tuple instantiated with all 6 components and Check evaluated"
entities:
  - tuple
  - constraint
  - invariant
...
## Command Ontology Integration

`/step` is the **orchestrator** that routes to other commands based on intent. It uses the command taxonomy defined in [metadata.yaml](./metadata.yaml).

### Intent-Based Routing

When `/step` receives a task, it analyzes intent and selects the default strategy:

| Intent Pattern | Detected By | Default Strategy |
|----------------|-------------|------------------|
| **Goal-oriented** | "implement", "build", "create", "add", "fix" | `[/step]` (self) |
| **Exploration** | "what are options", "how could we", "explore" | `[/explore]` |
| **Verification** | "is it correct", "verify", "check if" | `[/validate]` |
| **Deductive** | "prove", "must be true", "given constraints" | `[/proof]` |
| **Explanation** | "how does", "what is", "explain" | `[/understand]` |
| **Comparison** | "vs", "compare", "which is better" | `[/what-if]` |
| **Causal** | "why did", "root cause", "what caused" | `[/trace]` |
| **Comprehensive** | "analyze thoroughly", "full analysis" | `[/analysis]` |

### Composite Expansion

When routing to a **composite command**, `/step` expands it into primitives:

```yaml
# User: /step "analyze the timeout issue thoroughly"
# Detected intent: comprehensive
# Selected strategy: /analysis

# /step expands /analysis into:
strategy:
  - mode: "/explore"
    prompt: "explore timeout issue from all angles"
  - mode: "/what-if"
    prompt: "compare alternative explanations"
  - mode: "/validate"
    prompt: "test hypotheses with evidence"
  - mode: "/consolidate"
    prompt: "synthesize findings"
```

**Composite expansions** (from metadata.yaml):
...
| `/evolve` | observe → validate → propose_updates |

### Alias Resolution

When routing to an **alias**, `/step` resolves to the base command:

| Alias | Resolves To | Mode Override |
|-------|-------------|---------------|
| `/compare` | `/what-if` | multi_way |
| `/summary` | `/consolidate` | communication_focus |
| `/explain` | `/understand` | communication_focus |
| `/research` | `/explore` | thorough |
| `/impact` | `/trace` | forward |

### Domain Binding

When routing to a **domain-specialized command**, `/step` binds the domain context:

```yaml
# User: /step "replicate the hero animation"
# Detected: animation task
# Selected: /animate (domain-specialized)

# /step binds domain context:
strategy:
  - mode: "/animate"
    domain: web_motion
    base: dslp_pipeline
...
**Output mode commands** are NOT part of Strategy—they run **after** the Tuple completes:

```yaml
# After Check passes:
output_modes:
  - "/journal"   # Document what was learned
  - "/report"    # Summarize session
```

Output modes produce artifacts but don't modify Tuple state.
...
/step receives task
    ...
    ├── Analyze intent
    │
    ├── Select strategy (from routing table or explicit)
    │
    ├── If composite → expand to primitives
    │
    ├── If alias → resolve to base
    │
    ├── If domain-specialized → bind domain context
    │
    ├── Execute strategy pipeline
    │   ├── Each mode modifies Tuple state
    │   └── Check after each mode
    │
    ├── Final Check against Invariant
    │
    └── If output modes requested → execute after Check
```
...
- [Command Metadata](./metadata.yaml) - Complete command ontology
- [CLAUDE.md - Principle #26](./../CLAUDE.md) - Thinking Tuple Protocol
- [CLAUDE.md - Principle #27](./../CLAUDE.md) - Commands as Strategy Modes
